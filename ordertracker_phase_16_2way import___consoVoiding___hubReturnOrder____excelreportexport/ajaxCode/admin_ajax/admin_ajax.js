...
$(document).on('click','.getNAProdByIdNA',function(e) {
    e.preventDefault();
        ...
        $.ajax({
            ...
            success: function(response){
                ...
                Swal.fire({
                    ...
                    title: 'Order successfully consolidated!',
                    type: 'Success'
                })
...
$(document).on('click','#adminReportGenerateBtn',function(e) {
    e.preventDefault();

    var reportDate = $('#reportsdateStartAndEnd').val();//get input value

    $.ajax({
        url:'phpAction/admin_action/admin_action.php',// point to server-side PHP script 
        data: {
            'action' : "generateReports", //set an action to trigger , what if condition to be use.
            'reportDate' : reportDate
        },            
        type: 'POST',
        success: function(response){

           $('#orderReports').html(response);
           $('#orderReportTableView').DataTable({
            order: [0, 'desc'], 
                // use this to make the the appear the current record in the top
                dom: 'Bfrtip',
                buttons: [
                    'csv', 'excel'
                ]
...
$(document).on('click','#adminReportGenerateBtnForCusSas',function(e) {
    e.preventDefault();

    var reportDate = $('#reportsdateStartAndEndForCusSas').val();//get input value

    $.ajax({
        url:'phpAction/admin_action/admin_action.php',// point to server-side PHP script 
        data: {
            'action' : "generateReportsForCusSas", //set an action to trigger , what if condition to be use.
            'reportDate' : reportDate
        },            
        type: 'POST',
        success: function(response){

           $('#CutomerSatisfactionView').html(response);
           $('#orderReportForCusSasTableView').DataTable({
            order: [0, 'desc'], 
                // use this to make the the appear the current record in the top
                dom: 'Bfrtip',
                buttons: [
