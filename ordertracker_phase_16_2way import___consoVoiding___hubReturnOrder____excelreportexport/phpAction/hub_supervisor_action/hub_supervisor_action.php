                    ...
                    foreach ($rOrderData as $row){
                        ...
                        if (in_array($order_number, $foNumberArray, TRUE)){
                            ...
                            <td>
                                ...
                                <a href="#" title="Return Order To Consolidator" class="text-danger return_tohub_Btn" data-toggle="modal" data-target="#returnOrdermodal" orderNumber = "'.$row['order_number'].'" addressOne = "'.$row['c_address_one'].'" addressTwo = "'.$row['c_billing_add_one'].'"> <i class="fas fa-exchange-alt"></i>  </a>&nbsp;&nbsp;
...
if (isset($_POST['action']) && $_POST['action'] == "saveReturnOrder") {
    
   $orderNumber = $_POST['orderNumber'];
   $remarks = $_POST['remarks'];

   $orderDetails = $db->getOrderNumberDetails($orderNumber);

   $hubResult  = $db->getZipByHubNumber($orderDetails[0]['hub_area']);

   $zipcodeChecking = $db->zipcodeChecking($orderDetails[0]['c_zipcode']);


    // order details table hub value in the database will be not covered
    // then remove the zipcode in specific hub
    // insert the orderDetails variable in to order history table in the dastabase.

   if (count($zipcodeChecking) > 1){
       
        $db->updateHubToUndefined($orderDetails[0]['order_number']);

    }elseif (count($zipcodeChecking) == 1) {

        $db->updateHubToNotCovered($orderDetails[0]['order_number']);
   }

        $explodedZipcode = explode(",",$hubResult[0]['zip_code']);
        $arraySearch = $orderDetails[0]['c_zipcode'];
        $pos = array_search($arraySearch, $explodedZipcode);
        unset($explodedZipcode[$pos]);
        $implodedZipcode = implode(",",$explodedZipcode);
        $db->updateHubZipcode($implodedZipcode,$orderDetails[0]['hub_area']);

        $order_date = $orderDetails[0]['order_date'];
        $order_number = $orderDetails[0]['order_number'];
        $c_fullname = $orderDetails[0]['c_fullname'];
        $c_address_one = $orderDetails[0]['c_address_one'];
        $c_address_two = $orderDetails[0]['c_address_two'];
        $c_billing_add_one = $orderDetails[0]['c_billing_add_one'];
        $c_billing_add_two = $orderDetails[0]['c_billing_add_two'];
        $c_pnumber_primary = $orderDetails[0]['c_pnumber_primary'];
        $c_pnumber_one = $orderDetails[0]['c_pnumber_one'];
        $c_pnumber_two = $orderDetails[0]['c_pnumber_two'];
        $c_zipcode = $orderDetails[0]['c_zipcode'];
        $hub_area = $orderDetails[0]['hub_area'];
        $finalremarks = $remarks;

        $db->insertReturnHistoryDetails($order_date,
                                        $order_number,
                                        $c_fullname,
                                        $c_address_one,
                                        $c_address_two,
...
if (isset($_POST['action']) && $_POST['action'] == "getAndShowReturnProd") {
...
if (isset($_POST['action']) && $_POST['action'] == "getReturnOrderDetails") {
    ...
    $getReturnOrder = $db->getReturnOrder($ugetUserHubNUmber);
