...
if (isset($_POST['action']) && $_POST['action'] == "insertExcel") {
    ...
    if (!empty($_FILES)) {
        ...
        if (in_array($_FILES["file"]["type"], $allowedFileType)) {
            ...
            while(($filesop = fgetcsv($handle, 1000, ",")) !== false) {
                ...
                if ($ctr == 1) { 
                    $excelDataToSave[] = '"'.strip_tags(trim($filesop[0])).'"'.',';
                    $excelDataToSave[] = '"'.strip_tags(trim($filesop[1])).'"'.',';
                    $excelDataToSave[] = '"'.strip_tags(trim($filesop[2])).'"'.',';
                    $excelDataToSave[] = '"'.strip_tags(trim($filesop[3])).'"'.',';
                    $excelDataToSave[] = '"'.strip_tags(trim($filesop[4])).'"'.',';
                    $excelDataToSave[] = '"'.strip_tags(trim($filesop[5])).'"'.',';
                    $excelDataToSave[] = '"'.strip_tags(trim($filesop[6])).'"'.',';
                    $excelDataToSave[] = '"'.strip_tags(trim($filesop[7])).'"'.',';
                    $excelDataToSave[] = '"'.strip_tags(trim($filesop[8])).'"'.',';
                    $excelDataToSave[] = '"'.strip_tags(trim($filesop[9])).'"'.',';
                    $excelDataToSave[] = '"'.strip_tags(trim($filesop[10])).'"'.',';
                    $excelDataToSave[] = '"'.strip_tags(trim($filesop[11])).'"'.',';
                    $excelDataToSave[] = '"'.strip_tags(trim($filesop[12])).'"'.',';
                    $excelDataToSave[] = '"'.strip_tags(trim($filesop[13])).'"'.',';
                    $excelDataToSave[] = '"'.strip_tags(trim($filesop[14])).'"'.',';
                    $excelDataToSave[] = '"'.strip_tags(trim($filesop[15])).'"'.',';
                    $excelDataToSave[] = '"'.strip_tags(trim($filesop[16])).'"'.',';
                    $excelDataToSave[] = '"'.strip_tags(trim($filesop[17])).'"'.',';
                    $excelDataToSave[] = '"'.strip_tags(trim($filesop[18])).'"'.',';
                    $excelDataToSave[] = '"'.strip_tags(trim($filesop[19])).'"'.',';
                    $excelDataToSave[] = '"'.strip_tags(trim($filesop[20])).'"'.',';
                    $excelDataToSave[] = '"'.strip_tags(trim($filesop[21])).'"'.',';
                    $excelDataToSave[] = '"'.strip_tags(trim($filesop[22])).'"'.',';
                    $excelDataToSave[] = '"'.strip_tags(trim($filesop[23])).'"'.',';
                    $excelDataToSave[] = '"'.strip_tags(trim($filesop[24])).'"'.',';
                    $excelDataToSave[] = '"'.strip_tags(trim($filesop[25])).'"'.',';
                    $excelDataToSave[] = '"'.strip_tags(trim($filesop[26])).'"'.',';
                    $excelDataToSave[] = '"'.strip_tags(trim(isset($filesop[27]) ? $filesop[27] : "Hub area")).'"'."\n";
                ...
                $hub_colum = strip_tags(trim(isset($filesop[27]) ? $filesop[27] : ""));
                
                // trapping condition start
                $no_double_order_checking = $db->orderDoubleCheck(
                    $order_number,
                    $property_note,
                    $variant_name,
                    $product_name,
                    ...
                    if ($order_date == "1970/01/01" || $order_date == "") {
                        ...
                        $zipcodeChecking = $db->zipcodeChecking($c_zipcode);
                        ...
                        if(!empty($hubColumn)){
                            ...
                            $excelDataToSave[] = '"'.$order_date.'"'.',';
                            $excelDataToSave[] = '"'.$order_number.'"'.',';
                            $excelDataToSave[] = '"'.$c_fullname.'"'.',';
                            $excelDataToSave[] = '"'.$c_pnumber_one.'"'.',';
                            $excelDataToSave[] = '"'.$c_pnumber_two.'"'.',';
                            $excelDataToSave[] = '"'.$c_pnumber_primary.'"'.',';
                            $excelDataToSave[] = '"'.$c_email.'"'.',';
                            $excelDataToSave[] = '"'.$order_note.'"'.',';
                            $excelDataToSave[] = '"'.$c_address_one.'"'.',';
                            $excelDataToSave[] = '"'.$c_address_two.'"'.',';
                            $excelDataToSave[] = '"'.$c_billing_add_one.'"'.',';
                            $excelDataToSave[] = '"'.$c_billing_add_two.'"'.',';
                            $excelDataToSave[] = '"'.$c_zipcode.'"'.',';
                            $excelDataToSave[] = '"'.$c_city.'"'.',';
                            $excelDataToSave[] = '"'.$o_total_price.'"'.',';
                            $excelDataToSave[] = '"'.$o_status.'"'.',';
                            $excelDataToSave[] = '"'.$o_cancelled_at.'"'.',';
                            $excelDataToSave[] = '"'.$o_payment_gateway.'"'.',';
                            $excelDataToSave[] = '"'.$o_special_discount.'"'.',';
                            $excelDataToSave[] = '"'.$o_card_id_number.'"'.',';
                            $excelDataToSave[] = '"'.$o_card_discount.'"'.',';
                            $excelDataToSave[] = '"'.$o_card_discount_number.'"'.',';
                            $excelDataToSave[] = '"'.$property_note.'"'.',';
                            $excelDataToSave[] = '"'.$variant_price.'"'.',';
                            $excelDataToSave[] = '"'.$variant_name.'"'.',';
                            $excelDataToSave[] = '"'.$product_name.'"'.',';
                            $excelDataToSave[] = '"'.$total_quantity_sold.'"'.',';
                            $excelDataToSave[] = $hubNumber."\n";
                        ...
                        elseif (count($zipcodeChecking) > 1){
                            $hubNumber = "undefined";
                        ...
                        elseif(count($zipcodeChecking) == 1){ 
                            $hubNumber = $zipcodeChecking[0]['hub_area'];
                        ...
                        elseif (empty($zipcodeChecking)){
                            $hubNumber = "not covered";
    ...
    if(count($foOrderNumber) == 0){
        ...
        $db->orderImportHistoryData($excelFinalName,$hubNumber,$orderNumbers,$cNames,$fnameAndLname,$uID);
    }else{
...
if (isset($_POST['action']) && $_POST['action'] == "insertAccount") {
    ...
    if(!empty($data)){
        ...
        if($urole == "Consolidator"){
            $uhubnumber = "";

            if($uhubnumber == "" || !empty($uhubnumber)){
                $uhubnumber = "";
            }
        }
...
if (isset($_POST['action']) && $_POST['action'] == "getHubShipper") {
    ...
    if (isset($_POST['action']) && $_POST['action'] == "showAllOrderDetails") {
        ...
        $order_number = '';
        $foNumberArray = [];
        $forDelivery = 'transit'; // admin show all pending order (all hub)
...
// dropDown for hub List CodeuserTryToLogin
          ...
          <tbody>';
                ...
                foreach ($filterByOstatusAndDate as $row){
                    ...
                    if (in_array($order_number, $foNumberArray, TRUE)){
                        ...
                        $shipperFullName = $db->getAccountId($row['shipper_user_id']);
        ...
        $importHistoryData = $db->importHistoryDataView();
            ...
            foreach($importHistoryData as $row){
                    ...
                    if (strlen($filename) > 27){
                       $limitFileNameCharacter =  substr($filename,0,27) . '...';
                    }else{
                        $limitFileNameCharacter = $row['file_name'];
                    }

                    $output .= '<tr  class = "text-center text-secondary">
                    <td>'.$row['id'].'</td>
                    <td>'.$limitFileNameCharacter.'</td>
                ...
                <table id="NaProdTable" class="display responsive nowrap table table-striped table-bordered" width = "100%">
                  ...
                  <tbody>';
                        ...
                        foreach ($NAItems as $row){
                                ...
                                <td>'.$row['hub_area'].'</td>
                                <td>'.$row['order_number'].'</td>
                                <td> <a href="#" title="Customer Details" class="text-info getNAProdByIdNA" data-toggle="modal" data-target="#NAProdModal" id = "'.$row['id'].'"> <i class = "fas fa-info-circle fa-md" ></i>  </a></td>
    ...
    if (isset($_POST['action']) && $_POST['action'] == "showUnconsoOrders") {

        $output = '';
      
        $foNumberArray = []; // admin show all pending order (all hub)
    
        $rOrderData = $db->readAllUncosoOrders();
    
                $output .= '
                <table id="unconsoOrderTableView" class="display responsive nowrap table table-striped table-bordered" width = "100%">
                  ...
                  <tbody>';
                        ...
                        foreach ($rOrderData as $row){
                            ...
                            elseif($o_status == "for checking" || $o_status == "for delivery" || $o_status == "paid" || $o_status == "voided"){
                                continue;
                            }
                            else{
                                $foNumberArray[] = $order_number;
                                ...
                                <td>
                                    <a href="#" title="Customer Order Details" class="text-primary customer_Order_Info_Btn" data-toggle="modal" data-target="#Customer_Order_details_modal" orderNumber = "'.$row['order_number'].'"> <i class = "fas fa-shopping-cart fa-md" ></i>  </a> &nbsp;&nbsp;
                                    <a href="#" title="Customer Details" class="text-info customer_Info_Btn" data-toggle="modal" data-target="#Customer_details_modal" id = "'.$row['id'].'" ordernumber = "'.$row['order_number'].'"> <i class = "fas fa-info-circle fa-md" ></i>  </a> &nbsp;&nbsp;
                                    <a href="#" title="Cancel Order" class="text-danger cancel_order_Btn" data-toggle="modal" data-target="#cancelOrdermodal" orderNumber = "'.$row['order_number'].'"> <i class = "fas fa-window-close fa-md" ></i></a>
                                </td>
                              </tr>';
    ...
    if (isset($_POST['action']) && $_POST['action'] == "assignUncoOrderToHub") {
        ...
        $orderNumber = $_POST['orderNumber'];
        $hubArea = $_POST['hubArea'];

        //update the hub_area code kase ang value is "not coverd" or "undefined"
        $getOrderByOrderNumbers = $db->getOrderByOrderNumbers($orderNumber);
        $hubAreaValue = $getOrderByOrderNumbers[0]['hub_area'];
        $newZipcode = $getOrderByOrderNumbers[0]['c_zipcode'];

            if ($hubAreaValue == "not covered"){
                //update the hub_area
                //update the hub_area_number table in the database kase hindi pa na save nung zipcode sa hub_area_number table

                $gettedHubArea = $db->gettedHubArea($hubArea);

                $HubAreaZipcode = $gettedHubArea[0]['zip_code'];

                $finalZipcodeToSave = $HubAreaZipcode . ',' . $newZipcode;

                $db->updateHubAreaZipCode($finalZipcodeToSave,$hubArea);

                $db->updateHubBecauseItsNoHub($hubArea,$orderNumber);
            
            }elseif ($hubAreaValue == "undefined"){
                //update the hub_area dahil undefined. zipcode hindi kase nakasave na nung zipcode

                $db->updateHubBecauseItsNoHub($hubArea,$orderNumber);
            }
        // END update the hub_area code kase ang value is not coverd or undefined
    ...
    if (isset($_POST['action']) && $_POST['action'] == "getOrderDetailsForVoided") {
        ...
        $orderDetails = $db->getCustomerOderDetailCheckedWhoVoid($ordernumber);

        if (!empty($orderDetails[0]['consolidator_date_time_voided_order'])){

            $order_date = date("m/d/Y",strtotime($orderDetails[0]['order_date']));
            $order_number = $orderDetails[0]['order_number'];

            if ($orderDetails[0]['hub_area'] == "not covered" || $orderDetails[0]['hub_area'] == "undefined"){
                $hub_area = "none";
            }else{
                $hub_area = $orderDetails[0]['hub_area'];
            }
            
            $shipperName = "none";
            $dispatchedDateTime = "none";
            $deliveryDateTime = "none";
            $deliveredDateTime = "none";

            
            // Voided by:
            $dateVoidedConso = date("m/d/Y H:i:s",strtotime($orderDetails[0]['consolidator_date_time_voided_order']));
            $ConsoRemarks = $orderDetails[0]['remarks'];

            $dateVoidedHub = "none";
            $HubRemarks = "none";

            $dateVoidedShipper = "none";
            $ShipperRemarks = "none";
            // end Voided by:

        }
        elseif(!empty($orderDetails[0]['hub_pending_date_time_voided'])){
            $order_date = date("m/d/Y",strtotime($orderDetails[0]['order_date']));
            $order_number = $orderDetails[0]['order_number'];
            $hub_area = $orderDetails[0]['hub_area'];
            ...
            // Voided by:
            $dateVoidedConso = "none";
            $ConsoRemarks = "none";

            $dateVoidedHub = date("m/d/Y H:i:s",strtotime($orderDetails[0]['hub_pending_date_time_voided']));
            $HubRemarks = $orderDetails[0]['remarks'];

            $dateVoidedShipper = "none";
            $ShipperRemarks = "none";
            // end Voided by:
        }
        elseif(!empty($orderDetails[0]['shipper_for_voided_date_time'])){
            $shipperId= $orderDetails[0]['shipper_user_id'];

            $shipperDetails = $db->selectShipperByID($shipperId);

            $shipperFullName = $shipperDetails[0]['first_name'] . ' ' . $shipperDetails[0]['last_name'];

            $order_date = date("m/d/Y",strtotime($orderDetails[0]['order_date']));
            $order_number = $orderDetails[0]['order_number'];
            $hub_area = $orderDetails[0]['hub_area'];

            $shipperName = $shipperFullName;
            $dispatchedDateTime = date("m/d/Y H:i:s",strtotime($orderDetails[0]['hub_order_dispatched_date_time']));
            $deliveryDateTime = date("m/d/Y H:i:s",strtotime($orderDetails[0]['shipper_for_delivery_date_time']));
            $deliveredDateTime = "none";
            ...
            $dateVoidedShipper = date("m/d/Y H:i:s",strtotime($orderDetails[0]['shipper_for_voided_date_time']));
            $ShipperRemarks = $orderDetails[0]['remarks'];
            // end Voided by:
        ...
        $output .= '
                    <ul>
                        ...
                        <li>Hub area: '.$hub_area.'</li> 
                        <li>Shipper name: '.$shipperName.' </li> 
                        <li>Date time (dispatched): '.$dispatchedDateTime.'</li>
                        <li>Date time (delivery): '.$deliveryDateTime.'</li>
                        <li>Date time (delivered): '.$deliveredDateTime.'</li>

                        <br>    
                        
                        <p>Voided by: </p>
                        <li>Consolidator: '.$dateVoidedConso.' </li>
                        <p>Remark: '.$ConsoRemarks.' </p>

                        <li>Hub supervisor: '.$dateVoidedHub.'</li>
                        <p>Remark: '.$HubRemarks.'</p>

                        <li>Shipper: '.$dateVoidedShipper.'</li>
                        <p>Remark: '.$ShipperRemarks.'</p>
                    </ul>
                    ';
    ...
    if (isset($_POST['action']) && $_POST['action'] == "getReturnOrderDetails") {

    
        $getReturnOrder = $db->getReturnOrder();
    
        $output = '';
    
        $output .= '
                <table id="returnOrderTableView" class="display responsive nowrap table table-striped table-bordered" width = "100%">
                  ...
                  <tbody>';
                        ...
                        foreach ($getReturnOrder as $row){
    
                                $output .= '<tr  class = "text-center text-secondary">
                                <td>'.$row['id'].'</td>
                                <td>'.$row['order_number'].'</td>
                                <td>'.$row['c_address_one'].'</td>
                                <td>'.$row['c_billing_add_one'].'</td>
                                <td>'.$row['c_zipcode'].'</td>
                                <td>'.date("m/d/Y H:i:s",strtotime($row['date_returned'])).'</td>
                                <td>'.$row['hub_area'].'</td>
                                <td>'.$row['remarks'].'</td>
                              </tr>';
    ...
    if (isset($_POST['action']) && $_POST['action'] == "generateReports") {

        $reportDate = $_POST['reportDate'];

        $explodedReportDate = explode(" - ",$reportDate);

        $explodeStartDate = date("Y/m/d", strtotime($explodedReportDate[0]));
        $explodeEndtDate = date("Y/m/d", strtotime($explodedReportDate[1]));

        $explodeStartDate1 = $explodeStartDate;
        $explodeEndtDate2 = $explodeEndtDate;
       
        $reportResult = $db->reportResult($explodeStartDate1,$explodeEndtDate2);
        $order_number = '';
        $foNumberArray = []; // admin show all pending order (all hub)
        ...
        $output .= '
                <table id="orderReportTableView" class="display responsive nowrap table table-striped table-bordered" width = "100%">
                  ...
                  <tbody>';
                        ...
                        if (in_array($order_number, $foNumberArray, TRUE)){
                            continue;
                                ...
                                <td>'.$row['hub_area'].'</td>
                                <td>'.$row['hub_order_dispatched_date_time'].'</td>
                                <td>'.$row['shipper_for_delivery_date_time'].'</td>
                                <td>'.$row['shipper_for_delivered_date_time'].'</td>
                                <td>'.$row['shipper_for_voided_date_time'].'</td>
                                <td>'.$row['hub_pending_date_time_voided'].'</td>
                                <td>'.$row['consolidator_date_time_voided_order'].'</td>
                                <td>'.$row['remarks'].'</td>
                                
                              </tr>';

                              $foNumberArray[] = $row['order_number'];
                            }  
                        }
                            $output .= '</tbody> </table>';
    ...
    if (isset($_POST['action']) && $_POST['action'] == "generateReportsForCusSas") {
        ...
        $reportResult = $db->reportResultForCusSas($explodeStartDate1,$explodeEndtDate2);
        $order_number = '';
        $foNumberArray = []; // admin show all pending order (all hub)
        ...
        $output .= '
                <table id="orderReportForCusSasTableView" class="display responsive nowrap table table-striped table-bordered" width = "100%">
