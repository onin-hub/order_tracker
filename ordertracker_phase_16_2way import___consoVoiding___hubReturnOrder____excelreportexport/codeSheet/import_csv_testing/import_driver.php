...
if (!empty($_FILES)) {
    ...
    if (in_array($_FILES["file"]["type"], $allowedFileType)) {
        ...
        while(($filesop = fgetcsv($handle, 1000, ",")) !== false) {
            ...
            $udepartment = strip_tags(trim(strtoupper($filesop[1])));
            $salary = strip_tags(trim($filesop[2]));
            ...
            if (!empty($name) && !empty($udepartment) && !empty($salary)) {

                // insert query here
                $result = $sampleInsert->insertSample($name, $udepartment, $salary);
