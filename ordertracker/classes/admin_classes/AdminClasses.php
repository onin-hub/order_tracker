...
public function reportResult($explodeStartDate1,$explodeStartDate2){

    $data = array(); //declare the variable to store the data

    $sql = "SELECT * FROM `order_details` WHERE `order_date` BETWEEN :explodeStartDate AND :explodeEndtDate";
    $stmt = $this->conn->prepare($sql);
    $stmt->execute([
        'explodeStartDate'=>$explodeStartDate1,
        'explodeEndtDate'=>$explodeStartDate2
        ]);
    $results = $stmt->fetchAll(PDO::FETCH_ASSOC);
...
public function reportResultForCusSas($explodeStartDate1,$explodeStartDate2){

    $data = array(); //declare the variable to store the data

    $sql = "SELECT * FROM `customer_satisfaction_data` WHERE `date_create` BETWEEN :explodeStartDate AND :explodeEndtDate";
    $stmt = $this->conn->prepare($sql);
    $stmt->execute([
        'explodeStartDate'=>$explodeStartDate1,
        'explodeEndtDate'=>$explodeStartDate2
