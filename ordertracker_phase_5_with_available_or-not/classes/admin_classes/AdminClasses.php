...
public function filterByOstatusHubAndDateForvoided($hubnumber,$orderStatus,$dateStart,$dateEnd){
    ...
    $sql = "SELECT * FROM `filterbyostatusanddateforvoided` WHERE `shipper_for_voided_date` BETWEEN :currentDateStart AND :currentDateEnd AND `o_status` = :o_status AND `hub_area` = :hubnumber";
    $stmt = $this->conn->prepare($sql);
    $stmt->execute([
        'currentDateStart'=>$dateStart,
        'currentDateEnd'=>$dateEnd,
...
public function filterByOstatusAndDateForvoided($orderStatus,$dateStart,$dateEnd){
    ...
    $sql = "SELECT * FROM `filterbyostatusanddateforvoided` WHERE `shipper_for_voided_date` BETWEEN :currentDateStart AND :currentDateEnd AND `o_status` = :o_status";
    $stmt = $this->conn->prepare($sql);
    $stmt->execute([
        'currentDateStart'=>$dateStart,
        'currentDateEnd'=>$dateEnd,
