...
public function filterByOstatusHubAndDateForvoided($hubnumber,$orderStatus,$dateStart,$dateEnd){
    ...
    $sql = "SELECT * FROM `filterbyostatusanddateforvoided` WHERE `shipper_for_voided_date` BETWEEN :dateStart AND :dateEnd AND `hub_area` = :hubnumber AND `o_status` = :orderStatus";
    $stmt = $this->conn->prepare($sql);
    $stmt->execute([
        'dateStart'=>$dateStart,
        'dateEnd'=>$dateEnd,
...
public function filterByOstatusHubZipAndDateForvoided($hubnumber,$orderStatus,$dateStart,$dateEnd,$zipCode){
    ...
    $sql = "SELECT * FROM `filterbyostatusanddateforvoided` WHERE `	shipper_for_voided_date` BETWEEN :dateStart AND :dateEnd AND `hub_area` = :hubnumber AND `o_status` = :orderStatus AND `c_zipcode` = :zipcode";
    $stmt = $this->conn->prepare($sql);
    $stmt->execute([
        'dateStart'=>$dateStart,
        'dateEnd'=>$dateEnd,
